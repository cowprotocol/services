...
- dial9 runtime telemetry (post-hoc Tokio flight recorder, autopilot only): always compiled in — `.cargo/config.toml` builds the whole workspace with `--cfg tokio_unstable` and dial9 is an unconditional autopilot dep. Inert at runtime unless `DIAL9_ENABLED=true`; uploads traces to S3. See [docs/DIAL9.md](docs/DIAL9.md)
