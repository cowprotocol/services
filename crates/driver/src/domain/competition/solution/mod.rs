...
impl Solution {
    ...
    pub async fn verify(
        ...
        if eth.balance(self.solver.address()).await? < gas.required_balance() {
            return Err(Error::InsufficientBalance);
        }

        // TODO Does this need to be done as part of merged settlement verification as
        // well? I think it's not worth bothering with this.
    ...
    async fn simulate(
        ...
        let tx = settlement
            .tx(eth.contracts().settlement(), internalization)
...
pub enum Error {
    ...
    InsufficientBalance,
    #[error("settlement solutions are not disjoint during merging")]
    DuplicateSolutions,
