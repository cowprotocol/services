...
impl SolverScore {
    pub fn merge(&self, other: SolverScore) -> Result<Self> {
        match (self, other) {
            (SolverScore::Solver(a), SolverScore::Solver(b)) => Ok(SolverScore::Solver(a + b)),
            (SolverScore::RiskAdjusted(a), SolverScore::RiskAdjusted(b)) => {
                Ok(SolverScore::RiskAdjusted(a * b))
            }
            (SolverScore::Surplus(a), SolverScore::Surplus(b)) => Ok(SolverScore::Surplus(a + b)),
            _ => Err(anyhow!("Cannot merge scores of different types")),
        }
    }
}
