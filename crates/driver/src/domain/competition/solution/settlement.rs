...
impl Settlement {
    ...
    pub(super) async fn encode(
        ...
        for trade in solution.trades.iter() {
            let trade::Execution { sell, buy } =
                trade.execution(&solution).map_err(|_| Error::AssetFlow)?;
...
/// uses to refer both to the settlement and the winning solution. This ID is
