    ...
    pub fn allowances(&self) -> Vec<eth::allowance::Required> {
        match self {
            ...
            Interaction::Liquidity(interaction) => {
                ...
                // As a gas optimization, we always approve the max amount
                // possible. This minimizes the number of
                // approvals necessary, and therefore
                // minimizes the approval fees over time. This is a
                // potential security issue, but we assume that the router
                // contract for protocol indexed liquidity to be
                // safe.
                vec![
                    eth::Allowance {
