...
pub struct Competition {
    ...
    /// Cached fast-path quote solutions. Unlike `/solve` (which caches
    /// ready-to-submit settlements), a quote runs against a throwaway auction
    ...
    pub quote_solutions: Cache<crate::domain::quote::Id, CachedQuoteSolution>,
...
impl Competition {
    ...
    pub async fn solve(&self, request: Request<Body>) -> Result<Vec<Solved>, Error> {
        ...
        let solutions = self
            ...
            .inspect_err(|err| {
                if err.is_timeout() {
                    notify::solver_timeout(&self.solver, auction.auction_id());
                }
            })?;
        ...
        let solutions = solutions.into_iter().filter(|solution| {
            if !ids.insert(solution.id().clone()) {
                ...
                notify::duplicated_solution_id(&self.solver, auction.auction_id(), solution.id());
                false
            } else {
                true
            }
        ...
        let solutions = solutions.filter(|solution| {
            if solution.is_empty(auction.surplus_capturing_jit_order_owners()) {
                ...
                notify::empty_solution(&self.solver, auction.auction_id(), solution.id().clone());
                false
            } else {
                true
            }
            ...
            .filter_map(|(solution, result)| async move {
                ...
                match result {
                    ...
                    Err(err) => {
                        ...
                        if !has_haircut {
                            notify::encoding_failed(&self.solver, auction.auction_id(), &id, &err);
                        }
                        None
                    }
                }
            ...
            .is_err()
        {
            ...
            notify::postprocessing_timed_out(&self.solver, auction.auction_id())
        }
    ...
    /// Caches a fast-path quote's solution (with its auction) under the
    /// quote's id for later settlement via `/settle`.
    ...
    /// Re-encode the quote solution cached under `quote_id` against the real
