...
impl Solver {
    ...
    pub async fn solve(
        ...
        // Only auctions with IDs are real auctions (/quote requests don't have an
        // ID). Only for those does it make sense to archive them and measure the
        // execution time.
        let has_id = auction.id().is_some();
        // We only need the body fully materialized in memory when it has to be
        // uploaded to S3. Otherwise we stream the serialized JSON straight into
        // the request body so the whole (potentially multi-MB) payload is never
        // held at once.
        let archive_id = self
            .persistence
            .archives_enabled()
        ...
        let body: reqwest::Body = match archive_id {
            Some(id) => {
                // Stream the request body while capturing a gzipped copy for S3,
                // so neither the request nor the archive holds the full JSON.
