...
pub struct S3 {
    ...
    pub s3_instance_upload_bucket: String,

    /// Prepended to the auction id to form the final instance filename on S3.
    /// Something like "staging/mainnet/"
    pub s3_instance_upload_filename_prefix: String,
}

impl From<S3> for s3::Config {
    fn from(value: S3) -> Self {
        Self {
            bucket: value.s3_instance_upload_bucket,
            filename_prefix: value.s3_instance_upload_filename_prefix,
        }
    }
}
