use {
    crate::{
        ...
        db::{get_checkpoint, set_checkpoint, uniswap_v3 as db},
    },
...
impl UniswapV3Indexer {
    ...
    pub async fn catch_up(&self, from_block: u64) -> Result<()> {
        ...
        set_checkpoint(&mut tx, &self.factory, from_block).await?;
    ...
    async fn prefetch_decimals(&self, logs: &[Log]) -> DecimalsCache {
        futures::stream::iter(pool_created_token_addresses(self.factory, logs))
            .map(|token| async move {
                let dec = super::fetch_decimals(&self.provider, token).await;
                (token, dec)
            })
...
async fn run_decimals_backfill_pass(
    ...
    let mut stream = futures::stream::iter(tokens)
        .map(|token| async move {
            ...
            // `IS NULL` filter on the next pass skips it.
            let dec = super::fetch_decimals(provider, token)
                .await
                .map(i16::from)
                .unwrap_or(-1);
            (token, dec)
