...
impl<T: Transport> Debug<T> {
    ...
    /// This is useful for detecting which storage slots are accessed during a call.
    pub fn trace_call(
        &self,
        call: CallRequest,
        block: BlockId,
...
pub struct StructLog {
    ...
    /// Stack values can be variable length hex strings, so we deserialize as strings
    #[serde(default)]
...
/// Custom deserializer for stack values that handles variable-length hex strings
/// (side note: I don't know why this has to be so complicated...)
