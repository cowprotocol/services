...
impl Dex {
    ...
    pub async fn swap(
        ...
        let swap = match self {
            // TODO Change the other ones to use auction
            Dex::Balancer(balancer) => balancer.swap(order, slippage, auction.gas_price).await?,
            Dex::OneInch(oneinch) => oneinch.swap(order, slippage, auction.gas_price).await?,
            Dex::ZeroEx(zeroex) => zeroex.swap(order, slippage, auction.gas_price).await?,
