...
impl Dex {
    ...
    pub async fn solve(&self, auction: auction::Auction) -> Vec<solution::Solution> {
        ...
        // TODO Should this be a field on Auction?
        ...
        for order in auction.orders.iter().cloned() {
            ...
            if let Some(solution) = self
                .solve_order(order, &prices, &auction)
                .instrument(span)
                .await
            {
    ...
    async fn solve_order(
        &self,
        order: order::Order,
        // TODO Prices are derived from Auction. Technically this can be a field on Auction. Might
        // be neater that way
