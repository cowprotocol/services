...
fn get_orders_by_uid_response(result: Result<BoxStream<'static, Result<Order>>>) -> Response {
    match result {
        Ok(stream) => {
            let orders = stream.filter_map(async |item| {
                item.inspect_err(|err| tracing::warn!(?err, "failed to fetch order"))
                    .ok()
            });
...
fn streaming_response(orders: impl Stream<Item = Order> + Send + 'static) -> Response {
    let jsonl_stream = orders
        .filter_map(async move |order| serde_json::to_string(&order).ok())
...
mod tests {
    ...
    async fn get_orders_by_uid_ok() {
        ...
        let response_orders: Vec<Order> = body
            .split(|&b| b == b'\n')
            .filter(|line| !line.is_empty())
            .map(|line| serde_json::from_slice(line).unwrap())
            .collect();
        assert!(response_orders.eq(&order));
    }
