...
fn full_order_into_model_order(order: FullOrder) -> Result<Order> {
    ...
    let metadata = OrderMetadata {
        ...
        full_app_data: order
            .full_app_data()
            .map(|app_data| String::from_utf8(app_data.clone()))
            .transpose()
    ...
    fn order_status() {
        ...
        assert_eq!(
            calculate_status(
                order_row()
                    .update_base(|base| {
                        base.kind = DbOrderKind::Sell;
                        base.sell_amount = BigDecimal::from(10_000);
                    })
                    .update_self(|order| {
                        order.sum_sell = BigDecimal::from(9_999);
                    })
            ),
            OrderStatus::Open
        );
        ...
        assert_eq!(
            calculate_status(
                order_row()
                    .update_base(|base| {
                        base.kind = DbOrderKind::Sell;
                        base.sell_amount = BigDecimal::from(2);
                    })
                    .update_self(|order| {
                        order.sum_sell = BigDecimal::from(3);
                        order.sum_fee = BigDecimal::from(1);
                    })
            ),
        ...
        assert_eq!(
            calculate_status(
                order_row()
                    .update_base(|base| {
                        base.kind = DbOrderKind::Buy;
                        base.buy_amount = BigDecimal::from(1);
                    })
                    .update_self(|order| {
                        order.sum_buy = BigDecimal::from(0);
                    })
            ),
            OrderStatus::Open
        );
        ...
        assert_eq!(
            calculate_status(
                order_row()
                    .update_base(|base| {
                        base.kind = DbOrderKind::Buy;
                        base.buy_amount = BigDecimal::from(10_000);
                    })
                    .update_self(|order| {
                        order.sum_buy = BigDecimal::from(9_999);
                    })
            ),
            OrderStatus::Open
        );
        ...
        assert_eq!(
            calculate_status(
                order_row()
                    ...
                    .update_self(|order| {
                        order.sum_buy = BigDecimal::from(1);
                    })
            ),
            OrderStatus::Fulfilled
        );
        ...
        assert_eq!(
            calculate_status(
                order_row()
                    ...
                    .update_self(|order| {
                        order.sum_sell = BigDecimal::from(1);
                    })
            ),
            OrderStatus::Cancelled
        );
        ...
        assert_eq!(
            calculate_status(
                order_row()
                    .update_base(|base| {
                        base.kind = DbOrderKind::Buy;
                        base.buy_amount = BigDecimal::from(2);
                        base.invalidated = true;
                    })
                    .update_self(|order| {
                        order.sum_buy = BigDecimal::from(1);
