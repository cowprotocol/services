...
impl Infrastructure {
    pub async fn new(signer: TestAccount) -> Self {
        // Using the NODE_HOST here kind of sucks but this is a workaround
        // for the alloy provider signing issue
        let provider = provider_with_account(NODE_HOST, signer.private_key());
        ...
        let fallback = GnosisSafeCompatibilityFallbackHandler::Instance::deploy(provider.clone())
            .await
            .unwrap();
        let factory = GnosisSafeProxyFactory::Instance::deploy(provider)
            .await
            .unwrap();
    ...
    pub async fn deploy_safe(
        ...
        let provider = provider_with_account(NODE_HOST, owners[0].private_key());
        let safe_proxy =
...
impl Safe {
    ...
    pub async fn deploy(owner: TestAccount, web3: &Web3) -> Self {
        ...
        // tests.
        let infra = Infrastructure::new(owner.clone()).await;
