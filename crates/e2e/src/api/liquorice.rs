...
mod api {
    ...
    impl LiquoriceApi {
        ...
        pub fn notification_route(
            ...
            warp::path!("intent-origin" / "notification")
                .and(warp::post())
...
impl DomainSeparator {
    pub fn new(chain_id: u64, contract_address: H160) -> Self {
        ...
        static DOMAIN_VERSION: LazyLock<[u8; 32]> = LazyLock::new(|| signing::keccak256(b"1"));
...
pub struct Eip712TypedLiquoriceSingleOrder {
    pub rfq_id: String,
    pub nonce: U256,
    pub trader: H160,
...
impl Eip712TypedLiquoriceSingleOrder {
    // TODO: update
    ...
    pub fn sign(
        ...
        let signature = SecretKeyRef::from(&SecretKey::from_slice(signer.private_key()).unwrap())
    ...
    pub fn hash_struct(&self) -> [u8; 32] {
        let order_data_part_1 = {
            ...
            hash_data[32..64].copy_from_slice(
                signing::keccak256(encode(&[Token::String(self.rfq_id.clone())]).as_slice())
                    .as_slice(),
            );
        ...
        // 0xd11023397b6e58bf8137e479bc552f06eb3b7527652528a047eae91bb391858d
...
mod tests {
    use {
        crate::api::liquorice::Eip712TypedLiquoriceSingleOrder,
        hex_literal::hex,
    ...
    #[test]
    fn test_order_hash() {
        let order = Eip712TypedLiquoriceSingleOrder {
            ...
            quote_expiry: U256::from(1715787259), // Example timestamp
