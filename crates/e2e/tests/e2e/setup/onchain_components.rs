...
impl OnchainComponents {
    ...
    /// We will only index events when they are 64 blocks old so we don't have to throw out indexed
    /// data on reorgs.
    /// This function executes enough transactions to ensure that all events before calling this
    /// function are old enough to be indexed.
