...
#[tokio::test]
#[ignore]
async fn local_node_single_replace_order_test_with_custom_app_data() {
    run_test(single_replace_order_test_with_custom_app_data).await;
}
...
async fn single_replace_order_test_with_custom_app_data(web3: Web3) {
    let mut onchain = OnchainComponents::deploy(web3.clone()).await;
