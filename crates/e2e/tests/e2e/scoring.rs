...
#[tokio::test]
#[ignore]
async fn local_node_surplus_scoring() {
    run_test(surplus_scoring).await;
}

async fn surplus_scoring(web3: Web3) {
    ...
    let solver_endpoint = colocation::start_baseline_solver(solver.address()).await;
    colocation::start_driver(
        ...
        vec![
            SolverEngine {
                ...
                endpoint: solver_endpoint.clone(),
            },
            SolverEngine {
    ...
    assert!(balance_after_a >= to_wei(5));
    assert!(balance_after_b >= to_wei(2));
}
