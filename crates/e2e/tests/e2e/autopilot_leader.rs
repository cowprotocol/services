use {
    e2e::setup::{run_test, to_wei, wait_for_condition, OnchainComponents, Services, TIMEOUT},
    ethrpc::Web3,
    std::time::Duration, tokio::time::timeout,
};
...
async fn autopilot_graceful_shutdown(web3: Web3) {
    ...
    assert!(wait_for_condition(TIMEOUT, || async { services.get_auction().await.auction.block > block}).await.is_err());
}
...
async fn dual_autopilot_take_over(web3: Web3) {
    ...
    assert!(wait_for_condition(Duration::from_secs(15), || async { services.get_auction().await.auction.block > block}).await.is_ok());
}
