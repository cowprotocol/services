...
#[tokio::test]
#[ignore]
async fn local_node_autopilot_non_leader_graceful_shutdown() {
    run_test(autopilot_non_leader_graceful_shutdown).await;
}
...
async fn autopilot_non_leader_graceful_shutdown(web3: Web3) {
    let mut onchain = OnchainComponents::deploy(web3).await;
    let [solver] = onchain.make_solvers(to_wei(1)).await;
    let services = Services::new(&onchain).await;
    let (manual_shutdown, control) = autopilot::run::Control::new_manual_shutdown();
...
async fn dual_autopilot_with_full_protocol(web3: Web3) {
    ...
    // Assert new auction has been made
    onchain.mint_block().await;
    assert!(
        wait_for_condition(TIMEOUT, || async {
            services.get_auction().await.auction.block > block
    ...
    onchain.mint_block().await;
    assert!(
        tokio::time::timeout(TIMEOUT, autopilot_leader)
            .await
            .is_ok()
    );
    tracing::info!("The leader has stepped down");

    // Assert new auctions are being made by new leader
    let block = services.get_auction().await.auction.block;

    tracing::info!("Waiting on new auction");
    assert!(
        wait_for_condition(TIMEOUT, || async {
            onchain.mint_block().await;
            services.get_auction().await.auction.block > block
