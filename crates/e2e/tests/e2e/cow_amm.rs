...
#[tokio::test]
#[ignore]
async fn local_node_auction_contains_native_prices_for_cow_amm_tokens() {
    run_test(auction_contains_native_prices_for_cow_amm_tokens).await;
}
...
async fn auction_contains_native_prices_for_cow_amm_tokens(web3: Web3) {
    let mut onchain = OnchainComponents::deploy(web3.clone()).await;

    let [solver] = onchain.make_solvers(to_wei(100)).await;
    let [bob, cow_amm_owner] = onchain.make_accounts(to_wei(1000)).await;

    let [dai, doge] = onchain
        .deploy_tokens_with_weth_uni_v2_pools(to_wei(300_000), to_wei(100))
        .await;
    ...
    // tokens for 2 orders in the same direction without having to worry about
    // getting the liquidity on-chain.
    doge.mint(onchain.contracts().gp_settlement.address(), to_wei(100_000))
        .await;
    ...
    let user_order = OrderCreation {
        sell_token: onchain.contracts().weth.address(),
        sell_amount: U256::exp10(17), // 0.1 WETH
        buy_token: doge.address(),
        buy_amount: to_wei(230), // 230 DOGE
        valid_to: model::time::now_in_epoch_seconds() + 300,
        kind: OrderKind::Sell,
        ..Default::default()
    }
    ...
    // Drive solution and check that the native price for DAI is available even tho
    // the user order is for DOGE.
    tracing::info!("Waiting for trade.");
    wait_for_condition(TIMEOUT, || async {
