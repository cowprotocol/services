...
async fn order_creation_checks_metadata_signer(web3: Web3) {
    ...
    let full_app_data = full_app_data_with_signer(adversary.address().to_alloy());
    ...
    let full_app_data = full_app_data_with_signer(trader.address().to_alloy());
    ...
    token_a.mint(safe.address().to_legacy(), to_wei(10)).await;
    ...
    order5.from = Some(safe.address().to_legacy());
