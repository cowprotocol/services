...
impl KmsSigner {
    ...
    async fn sign(&self, message: &[u8]) -> Result<Signature, Error> {
        let kms_error = |error: Box<dyn std::error::Error + Send + Sync>| Error::Kms {
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|error| kms_error(Box::new(error)))?;
        let bytes = response
            .signature()
            .ok_or_else(|| kms_error("the response carries no signature".into()))?;
        Signature::try_from(bytes.as_ref()).map_err(|_| kms_error("malformed signature".into()))
    }
}
