use {
    ...
    crate::{PriceEstimationError, QuoteIdAllocating},
    futures::{
...
pub struct CompetitionEstimator<T> {
    ...
    quote_ids: Option<Arc<dyn QuoteIdAllocating>>,
}
...
impl<T: Send + Sync + 'static> CompetitionEstimator<T> {
    ...
    pub fn with_quote_ids(self, quote_ids: Arc<dyn QuoteIdAllocating>) -> Self {
        Self {
    ...
    async fn allocate_quote_ids(&self) -> Result<Vec<QuoteId>, PriceEstimationError> {
        let Some(allocator) = &self.quote_ids else {
            return Ok(vec![]);
        };
