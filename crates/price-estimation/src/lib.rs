...
pub struct Query {
    ...
    /// Id of the quote a solver's estimate is stored under if it wins. Each
    /// solver is asked with its own id (assigned by the competition
    /// estimator), so a solver can later match the quote it produced to the
    /// order placed with it, and the driver caches a fast-path solution under
    /// it. `None` on the query handed to the competition estimator, and for
    /// price estimates that are not quotes.
...
/// Allocates the ids quotes get stored under, one per solver asked, so that
/// every solver can be sent the id its quote will have if it wins.
#[cfg_attr(any(test, feature = "test-util"), mockall::automock)]
pub trait QuoteIdAllocating: Send + Sync + 'static {
    fn allocate(&self, n: usize) -> BoxFuture<'_, anyhow::Result<Vec<model::quote::QuoteId>>>;
}
