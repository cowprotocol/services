    ...
    pub async fn update_settlement_details(
        ...
        if let Some(auction_data) = settlement_update.auction_data {
            ...
            if insert_succesful || matches!(auction_data.auction_id, AuctionId::Centralized(_)) {
                // save order executions for orders with solver computed fees (limit orders)
                // for limit orders, fee is determined by the solver therefore, when transaction
                // is settled onchain we calculate the fee and save it to DB
