...
impl SolvableOrdersCache {
    ...
    pub async fn update(&self, block: u64) -> Result<()> {
        ...
        let balance_queries: Vec<_> = orders.iter().map(Query::from_order).collect();
        let balances = self.balance_fetcher.get_balances(&balance_queries).await;
        let balances: HashMap<_, _> = balance_queries
            .into_iter()
            .zip(balances)
            .filter_map(|(query, balance)| Some((query, balance.ok()?)))
            .collect();
