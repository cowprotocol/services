...
impl Trade {
    ...
    pub fn surplus_in_ether(&self, prices: &auction::Prices) -> Result<eth::Ether, Error> {
        ...
        self.surplus_to_ether(surplus_amount, prices)
    }

    pub(crate) fn surplus_to_ether(
        &self,
        ...
        Ok(price.in_eth(eth::TokenAmount(surplus.0)))
    }

    pub(crate) fn fee_to_ether(
        &self,
        ...
        self.surplus_to_ether(eth::SurplusTokenAmount(fee_in_surplus_token), prices)
    }
    ...
    pub(crate) fn surplus_over_limit_price(&self) -> Result<eth::SurplusTokenAmount, error::Math> {
    ...
    pub(crate) fn surplus_over_limit_price_before_fee(
        &self,
    ) -> Result<eth::SurplusTokenAmount, error::Math> {
        let limit_price = PriceLimits {
            sell: self.sell.amount,
    ...
    pub(crate) fn surplus_token(&self) -> eth::TokenAddress {
        match self.side {
