...
pub struct Submitter {
    ...
    pub gas_parameters_of_last_tx: Option<GasPrice>,
...
fn calculate_submission_gas_price(
    gas_price_of_last_submission: Option<GasPrice>,
    ...
    let mut new_priority_fee_per_gas = START_PRIORITY_FEE_TIP as f64;
    ...
    if Some(newest_nonce) == nonce_of_last_submission {
        if let Some(GasPrice::Eip1559 {
            max_fee_per_gas: max_fee_per_gas_of_last_tx,
            max_priority_fee_per_gas: max_priority_fee_per_gas_of_last_tx,
        }) = gas_price_of_last_submission
        {
            new_max_fee_per_gas = f64::max(
                new_max_fee_per_gas,
                max_fee_per_gas_of_last_tx.to_f64_lossy() * GAS_PRICE_BUMP,
            );
            new_priority_fee_per_gas =
                max_priority_fee_per_gas_of_last_tx.to_f64_lossy() * GAS_PRICE_BUMP;
        }
    }
    if new_max_fee_per_gas > MAX_GAS_PRICE as f64 {
        tracing::warn!(
        ...
        new_max_fee_per_gas = f64::min(MAX_GAS_PRICE as f64, new_max_fee_per_gas);
    }
    new_priority_fee_per_gas = f64::min(new_priority_fee_per_gas, new_max_fee_per_gas);
    let gas_price = GasPrice::Eip1559 {
        max_fee_per_gas: U256::from_f64_lossy(new_max_fee_per_gas),
        max_priority_fee_per_gas: U256::from_f64_lossy(new_priority_fee_per_gas),
    };
    Ok(gas_price)
}
...
mod tests {
    ...
    fn test_calculate_submission_gas_price() {
        ...
        let expected_result = GasPrice::Eip1559 {
            max_fee_per_gas: U256::from_f64_lossy(max_fee_per_gas * GAS_PRICE_BUFFER_FACTOR),
            max_priority_fee_per_gas: U256::from(START_PRIORITY_FEE_TIP),
        };
        ...
        let gas_price_of_last_submission = GasPrice::Eip1559 {
            max_fee_per_gas: U256::from_f64_lossy(max_fee_per_gas_of_last_tx),
            max_priority_fee_per_gas: U256::from(START_PRIORITY_FEE_TIP),
        };
        let result = calculate_submission_gas_price(
        ...
        .unwrap();
        let expected_result = GasPrice::Eip1559 {
            max_fee_per_gas: U256::from_f64_lossy(max_fee_per_gas_of_last_tx * GAS_PRICE_BUMP),
            max_priority_fee_per_gas: U256::from_f64_lossy(
                START_PRIORITY_FEE_TIP as f64 * GAS_PRICE_BUMP,
            ),
        };
        assert_eq!(result, expected_result);
        ...
        .unwrap();
        let expected_result = GasPrice::Eip1559 {
            max_fee_per_gas: U256::from(MAX_GAS_PRICE),
            max_priority_fee_per_gas: U256::from(START_PRIORITY_FEE_TIP),
        };
        assert_eq!(result, expected_result);
    }
