...
impl SettlementRanker {
    ...
    pub async fn rank_legal_settlements(
        ...
        rated_settlements.retain(|(solver, rated_settlement, _)| {
            ...
            let objective_value_positive = objective_value.ge(&Ratio::zero());
            if !objective_value_positive {
                tracing::debug!(
                    "Solution rejected for negative objective value. Solver={}, objective value={}",
                    solver.name(),
                    objective_value
                );
                solver.notify_auction_result(
