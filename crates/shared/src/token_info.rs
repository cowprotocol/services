use {
    ...
    ethrpc::{
        Web3,
        alloy::{conversions::IntoAlloy, errors::handle_alloy_contract_error},
    },
    futures::{
        FutureExt,
...
impl TokenInfoFetcher {
    async fn fetch_token(&self, address: H160) -> Result<TokenInfo, Error> {
        ...
        Ok(TokenInfo {
            decimals: handle_alloy_contract_error(decimals)
                .map_err(|err| Error(err.to_string()))?,
            symbol: handle_alloy_contract_error(symbol).map_err(|err| Error(err.to_string()))?,
        })
    }
}
