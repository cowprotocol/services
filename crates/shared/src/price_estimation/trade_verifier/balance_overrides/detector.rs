...
impl Detector {
    ...
    pub fn new(web3: ethrpc::Web3, probing_depth: u8) -> Self {
        ...
        Self(Arc::new(Inner { web3, strategies }))
    }
    ...
    pub async fn detect(
        ...
        let overrides = hashmap! {
            token.address().into_legacy() => StateOverride {
                state_diff: Some(self.strategies.iter().map(|helper| helper.strategy.state_override(&holder, &helper.balance)).collect()),
                ..Default::default()
            },
        };
        let balance = token
    ...
    /// Detects the balance storage slot using debug_traceCall, similar to Foundry's `deal`.
    /// This traces a balanceOf call and finds which storage slot is accessed.
    async fn detect_with_trace(
        ...
        // Iterate through slots in reverse order (last accessed is most likely the balance)
    ...
    fn extract_sload_slots(&self, trace: &ethrpc::extensions::StructLogTrace) -> Vec<H256> {
    ...
    /// Verifies that a storage slot controls the balance by setting it and checking balanceOf.
