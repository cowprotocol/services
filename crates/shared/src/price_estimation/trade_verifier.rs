...
// This should only be kept around until we are confident that trade
// Verification works as expected. Drop this type when trade verification
// becomes mandatory.
/// Verifier that blindly trusts that the proposed trade is legit.
pub(super) struct NoopVerifier;

#[async_trait::async_trait]
impl TradeVerifying for NoopVerifier {
    async fn verify(
        &self,
        _query: &PriceQuery,
