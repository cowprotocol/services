...
mod tests {
    ...
    fn append_events_test_swap() {
        ...
        let pool = pools[&address].read_lock();
        assert_eq!(pool.state.tick, BigInt::from(3));
        assert_eq!(pool.state.liquidity, U256::from(2));
        assert_eq!(pool.state.sqrt_price, U256::from(1));
    }
    ...
    fn append_events_test_burn() {
        ...
        append_events(&mut pools, vec![event]);
        assert_eq!(
            pools[&address].read_lock().state.liquidity_net,
            BTreeMap::from([
