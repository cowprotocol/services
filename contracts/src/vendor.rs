...
#[rustfmt::skip]
pub fn run(artifacts_dir: &Path) -> Result<()> {
    let vendor = Vendor::new(artifacts_dir)?;
    ...
    vendor
        ...
        .github(
            "Solver7702Delegate",
            "cowprotocol/solver-7702-delegate/d2382297c6edee05b035ddcb285185d0938e038c/out/\
...
impl Vendor {
    fn new(artifacts_dir: &Path) -> Result<Self> {
        eprintln!(
            "vendoring contract artifacts to '{}'",
...
impl Src {
    ...
    fn fetch_json(self) -> Result<serde_json::Value> {
        let value = match reqwest::blocking::get(self.0)?.json::<serde_json::Value>()? {
            ...
            value => {
                eprintln!(
                    "warning: unexpected value format, expected an array or object, got {value:?}"
                );
                value
            }
        };
