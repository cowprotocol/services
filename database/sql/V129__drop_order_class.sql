...
--
-- V128 kept `orders.class` for one release so that the previous orderbook version, which still
-- writes the column, could insert orders alongside the new one during the rolling deploy. That
-- release has shipped, no deployed writer sets the column anymore, so the column, its type and
-- the index that used it can go.
--
